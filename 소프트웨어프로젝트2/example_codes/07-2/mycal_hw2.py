...
from calcFunctions import factorial, decToBin, binToDec, decToRoman, functions
from calcConstants import constants
...
class Calculator(QWidget):
    ...
    def buttonClicked(self):
        ...
        if key == '=':
            ...
            self.display.setText(functions[key](self.display.text()))
            # if key == functionList[0]:
            #     value = factorial(self.display.text())
            #     self.display.setText(value)
            #
            # elif key == functionList[1]:
            #     value = decToBin(self.display.text())
            #     self.display.setText(value)
            #
            # elif key == functionList[2]:
            #     value = binToDec(self.display.text())
            #     self.display.setText(value)

        elif key in constantList:
            self.display.setText(self.display.text() + constants[key])
            # if key == constantList[0]:
            #     value = self.display.text() + pi()
            #     self.display.setText(value)
            #
            # elif key == constantList[1]:
            #     value = self.display.text() + velocityOfLight()
            #     self.display.setText(value)
            #
            # elif key == constantList[2]:
            #     value = self.display.text() + velocityOfSound()
            #     self.display.setText(value)
            #
            # elif key == constantList[3]:
            #     value = self.display.text() + distanceFromSun()
            #     self.display.setText(value)
